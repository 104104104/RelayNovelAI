...
import pickle, datetime, MeCab, re
...
for p, data in enumerate(pair):
    ...
    for _ in range(int(epoch / len(pair))):#エポック
        for i in range(0, len(data), bs):#バッチごとの学習
            ...
            print("epoch:\t{} / {}\tloss:\t{}\ttime:\t{}".format(count, sum_epoch, loss, ed-st))#学習状況
            count+=1
...
#####テスト#####
test_sentence = "それ　は　ひどい　事故　で　、　僕　は　すぐ　に　入院　する　こと　が　決まっ　た　。"#本文中にある単語しか使えない！
print("\n",test_sentence,"\n↓\n")
t_li = []
for i in test_sentence.split("　"):
    t_li.append(dic[i])
t_variable = Variable(np.array([t_li], dtype="int32"))
dec_response = model(enc_words=t_variable, train=False)
d=dic.keys()
print("テスト結果：\t",end="")
